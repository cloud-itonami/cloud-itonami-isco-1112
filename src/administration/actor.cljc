(ns administration.actor
  "AdministrativeActor — the ISCO-08 1112 senior administrative office support actor as
  ...
  The unconditional invariant: the AdministrativeAdvisor can never directly
  commit a record or dispatch an office action the AdministrativeGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [administration.advisor :as advisor]
            [administration.governor :as governor]
            [administration.store :as store]))

(defn build-graph
  "Build a compiled AdministrativeActor graph. `store` implements
  `administration.store/Store`. `advisor` implements
  `administration.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:requester-id (:requester-id request)
                                    :regulation-id (:regulation-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
